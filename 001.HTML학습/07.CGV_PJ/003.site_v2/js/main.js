...
window.addEventListener("DOMContentLoaded", () => {
  ...
  // 클래스 on을 추가한다.
  // 2. 나머지 영화선택 메뉴 li에는 클래스 "on"을 삭제ㅐ
  // -> 결과적으로 css로 구현한 선택메뉴 일어서 있기가 구현된다.

  // 이벤트 대상 : .mlist ul > li

  //변경 대상 === 이벤트 대상 (this)
  // 이벤트 종류 : click
  // 변경대상이 여러개 이므로 for of 문을 사용한다.

  let sml = document.querySelectorAll(".mlist ul > li");

  console.log("영화선택리스트", sml);

  // 선택요소 들의 노드리스트 컬렉션을 모두 click설정함
  ...
  for (let x of sml) {
    ...
    x.onclick = () => {
      // 모든 요소 클래스 on 지우기
      for (let z of sml) z.classList.remove("on");
